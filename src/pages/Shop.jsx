...
export default function Shop() {
  ...
  const items = products.filter((p) => {
    ...
    // Now checks the specific 'location' field from your MockAPI objects
    const matchesLocation = location === 'All' || p.location === location;

    // --- UPDATED PRICE LOGIC ---
    // Added Number(p.price) to ensure comparison works with API strings
    let matchesPrice = true;
    const priceValue = Number(p.price);

    if (priceRange === '$10 - $20') matchesPrice = priceValue >= 10 && priceValue <= 20;
    else if (priceRange === '$20 - $35') matchesPrice = priceValue > 20 && priceValue <= 35;
    else if (priceRange === '$35+') matchesPrice = priceValue > 35;
