...
export default function Shop() {
  // --- LOGIC PLACEHOLDERS (Comment out when connecting Context) ---
  const [query, setQuery] = useState('');
  const [loading, setLoading] = useState(false);
  const [products] = useState([
    { id: 1, name: 'Ethiopian Yirgacheffe', price: 18.50, origin: 'Ethiopia' },
    { id: 2, name: 'Colombian Dark Roast', price: 15.00, origin: 'Colombia' },
    { id: 3, name: 'Sumatra Mandheling', price: 19.25, origin: 'Indonesia' },
  ]);

  // const { products, loading } = useProducts() // Uncomment this later
      ...
      <div className="max-w-7xl mx-auto grid gap-8 lg:grid-cols-[300px_1fr]">
        ...
        <aside className="lg:sticky lg:top-24 self-start rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
          <div className="mb-10">
            ...
            <label className="block text-sm font-semibold text-slate-400 mt-4 mb-3 uppercase tracking-wider">Find a product</label>
            <input
        ...
        <main className="space-y-8">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h2 className="text-4xl font-black tracking-tight leading-tight text-slate-900">Our Collection</h2>
                <p className="mt-3 text-slate-500 max-w-2xl leading-relaxed">
                  Carefully curated beans roasted to perfection. Explore our signature blends and single-origin specialties.
                </p>
              </div>
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-20 text-center shadow-xl">
              ...
              <p className="text-slate-500 mb-8">Try searching for something else or clear the search.</p>
              <button
                onClick={() => setQuery('')}
